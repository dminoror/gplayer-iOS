...
- (void)willMoveToWindow:(UIWindow *)newWindow {
    ...
    if (newWindow) {
        __weak DLProgressSlider *weakSelf = self;
        observer = [[NSNotificationCenter defaultCenter] addObserverForName:@"playerCacheProgress" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
            NSLog(@"playerCacheProgress");
            [weakSelf updateProgress:[note.object objectForKey:@"tasks"] totalBytes:[[note.object objectForKey:@"totalBytes"] unsignedIntegerValue]];
        }];
    }
    else {
